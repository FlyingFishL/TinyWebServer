...
//从状态机中行的读取状态，分别表示读取一个完整的行，行出错，行的数据尚且不完整
...
http_conn::HTTP_CODE http_conn::parse_headers(char* text)
{
    ...
    else if (strncasecmp(text, "Host:", 5) == 0)                       //处理Host头部信息
    {
        text += 5;
        text += strspn(text, "\t");
        m_host = text;
...
// 当得到一个完整正确的HTTP请求时，我们就分析目标文件的属性。如果目标文件存在， 对所有用户可读，且不是目录，则使用mmap将其映射内存地址m_file_address处，并告诉调用者获取文件成功 
...
bool http_conn::write() {
    ...
    while (true)
    {
        ...
        if (temp <= -1)
        { //如果TCP写缓存没有空闲空间，则等待下一轮EPOLLOUT事件。虽然在此期间，服务器无法立即接受到同一客户的下一个请求，但这可以保证连接的完整性
            if (errno == EAGAIN)
            {
